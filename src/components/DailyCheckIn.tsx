...
import { Dumbbell, Beer } from 'lucide-react';

interface DailyCheckInProps {
    initialWorkedOut: boolean;
    initialDrankAlcohol: boolean;
    onUpdate: (workedOut: boolean, drankAlcohol: boolean) => Promise<void>;
}
...
export default function DailyCheckIn({
    ...
    const handleToggle = async (type: 'workout' | 'alcohol') => {
        setIsUpdating(true);
        ...
        try {
            await onUpdate(newWorkedOut, newDrankAlcohol);
        } catch (error) {
            // Revert on error
            if (type === 'workout') {
